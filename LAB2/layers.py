...
class MaxPooling(Layer):
  ...
  # primijeti da metoda backward_params ovdje nije implementirana, jer pooling layer nema (customizable) parametre
...
# potpuno povezani sloj, najčešće dolazi na kraju mreže
class FC(Layer):
  def __init__(self, input_layer, num_outputs, name,
    ...
    self.name = name
    self.has_params = True
    self.last_input_activation = None
  ...
  def forward(self, inputs):
    ...
    # eventualno treba zapamtiti ovaj inputs, bit će potreban kada ćemo raditi backpropagation?
    
    # da dobijemo odgovarajuću dimenziju izlaza, potrebno je transponirati matricu težina
    self.last_input_activation = inputs
  ...
  def backward_params(self, grads):
    ...
    """
    # ovo je FCL, pa su njegovi parametri samo W i b -> grad_weights i grad_bias
    
    grad_weights = np.dot(grads.transpose(), self.last_input_activation)
    grad_bias = np.sum(grads,  axis = 0) # ovo je isto kao u prvom labosu jel, samo zbrajamo gradijent izlaza po redovima
    return [[self.weights, grad_weights], [self.bias, grad_bias], self.name]
...
class ReLU(Layer):
  def __init__(self, input_layer, name):
    ...
    self.name = name
    self.has_params = False
    self.last_input_activation = None # ovo bi moglo biti krivo
  ...
  def forward(self, inputs):
    ...
    """
    self.last_input_activation = inputs
    return np.maximum(0, inputs) # koristiti maximum umjesto max jer maximum vraća novo polje nad kojim je provedena ova operacija
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    grads[self.last_input_activation <= 0] = 0 # gradijent prolazi samo tamo gdje je aktivacija bila veća od 0, na ostalim mjestima je 0
    return grads
...
class SoftmaxCrossEntropyWithLogits():
  ...
  #u ovoj funkciji računamo softmax nad zadnjim slojem
  # te nad dobivemnim vrijednostima računamo CE loss 
  # ulaz funkcije softmax - logit (izlaz zadnjeg sloja)  
  def forward(self, x, y):
    ...
    """
    #pdb.set_trace()
    y_one_hot = y  # pretpostavka - y je već one hot
    y_out = stable_softmax(x)
  ...
  def backward_inputs(self, x, y):
    ...
    n = x.shape[0]
    return (stable_softmax(x) - y)/n # jer smo uzeli prosjek u unaprijednom prolazu
  
  # opet, ReLU sloj nema backward_params jer ReLU naravno nema customizable parametre
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    return 0.5 * self.weight_decay * np.power(np.linalg.norm(self.weights.ravel(),ord=2),2)
